...
### 3.1. Referência de Comandos

| Comando                   | Ambiente Alvo      | Ações Realizadas                                                                  |
| :------------------------ | :----------------- | :-------------------------------------------------------------------------------- |
| `pnpm instance:staging`   | Staging            | Baixa as alterações, compila as imagens e inicia os containers com `.env.staging` |
| `pnpm instance:prod`      | Produção           | Baixa as alterações, compila as imagens e inicia os containers com `.env.prod`    |
| `pnpm instance:prod:down` | Produção / Staging | Para e remove a topologia de containers em execução                               |

### 3.2. Passos Sequenciais de Release

Ao lançar uma nova versão, execute os seguintes passos no servidor de destino:
...
2. **Verificar os Arquivos de Ambiente:**
   Certifique-se de que o arquivo de ambiente de destino (por exemplo, `instance/.env.prod`) esteja preenchido e corresponda ao modelo `instance/.env.prod.example`.

3. **Orquestrar o Lançamento da Stack:**

   ```bash
   pnpm instance:prod
   ```

4. **Verificar a Saúde dos Containers:**
   ```bash
