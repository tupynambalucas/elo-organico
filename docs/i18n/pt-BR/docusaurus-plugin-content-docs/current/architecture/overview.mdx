...
Para uma lista de referência completa de scripts de orquestração, comandos de serviço e fluxos de trabalho da stack de desenvolvimento, consulte o **[Guia de Orquestração do Monorepo](./infra-orchestration-deploy/orchestration.mdx)**.
...
<DocCardList
  items={[
    ...
    },
    {
      ...
      description:
        'Cascateamento de configurações hierárquicas entre os contextos de workspace para promover reutilização.',
    },
    {
      type: 'link',
...
Para um guia completo e detalhado sobre estruturas de diretórios, modelos de Docker Compose padrão ouro e padrões multi-serviços avançados (incluindo os padrões Gateway e Distributed Stack), consulte as documentações dedicadas de [Padrões de Infraestrutura](./infra-orchestration-deploy/infrastructure.mdx) e [Guia de Implantação](./infra-orchestration-deploy/deployment.mdx).
