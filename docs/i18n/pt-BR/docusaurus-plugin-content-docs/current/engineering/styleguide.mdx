...
- **O Operador `void`:** Quando uma função assíncrona é chamada por seus efeitos colaterais e NÃO é aguardada (awaited), prefixe-a com `void` (ex: `void startServer()`). Isso signals execução não bloqueante intencional.
...
- **Estilização e Medidas Responsivas:** Use **CSS Modules** (`.module.css`). Classes utilitárias do Tailwind são permitidas dentro dos módulos via `@apply`. Adhere estritamente aos **Padrões de Responsividade** definidos na Seção 5.
