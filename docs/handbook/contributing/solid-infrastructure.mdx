...
- **Strict Env Segregation**: Do not pass a monolithic `.env` file containing all system secrets to every container. Instead, define separated environment files (e.g., `.env.gh-cli` and `.env.git-agent`) so that each container operates within its minimal security surface.
