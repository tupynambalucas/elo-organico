...
For a complete reference list of orchestration scripts, service commands, and dev stack workflows, please refer to the **[Monorepo Orchestration Guide](./infra-orchestration-deploy/orchestration.mdx)**.
...
For a comprehensive, deep-dive guide on directory structures, gold-standard Docker Compose blueprints, and advanced multi-service patterns (including the Gateway and Distributed Stack patterns), refer to the dedicated [Infrastructure Standards](./infra-orchestration-deploy/infrastructure.mdx) and [Deployment Guide](./infra-orchestration-deploy/deployment.mdx) documentation.
