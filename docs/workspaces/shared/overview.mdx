...
│   └── package.json     # Configuration package (@elo-organico/config)
└── ui/                  # (Future) Shared React UI component library
```

## Core Packages

### `@elo-organico/config` (Centralized Tooling & Quality)
...
- **Design Tokens Integration**: Imports color palettes, responsive typography, and animation scales directly from `@elo-organico/studio` to map them to functional React components.
- **Universal Reusability**: Components built here are fully decoupled from database and backend contexts, making them safe to import and render in:
  ...
  - `docs` (Custom interactive pages inside EloDocs)
...
The configurations inside the Shared workspace are consumed at the root and package levels. For example, the root `package.json` references `@elo-organico/config` to coordinate git hook preparation:
