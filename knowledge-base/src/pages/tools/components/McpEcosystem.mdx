...
*   **Traditional Server-Sent Events (SSE):** Establishes a persistent uni-directional stream via `GET /sse` and accepts client messages via `POST /messages`, piping events directly to the underlying subprocess stdin.
