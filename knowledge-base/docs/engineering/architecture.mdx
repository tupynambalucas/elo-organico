...
- **Mechanism:** Vite leverages native `resolve.tsconfigPaths` to resolve aliases directly to the internal packages' src (e.g., `@elo-instance/core` points to `instance/packages/core/src/index.ts`).
...
- **Hosting:** All production artifacts are designed for professional self-hosting on our own infrastructure, with the exception of the Knowledge Base, which is hosted on **GitHub Pages**.
