...
- **Styling & Responsive Measures:** Use **CSS Modules** (`.module.css`). Utility classes from Tailwind are allowed inside modules via `@apply`. **Crucially, the use of fixed measurements (e.g., `px`) is strictly forbidden** to ensure a fluid and accessible layout. Always use responsive units: `rem` for typography and spacing (where `1rem = 16px`), `%`, `vh`/`vw` for structural layouts, and CSS functions like `clamp()` for fluid scaling.
