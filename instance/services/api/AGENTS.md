...
This workspace (`@elo-instance/api`) contains the Fastify 5 REST API for community-specific instance operations.
...
- **[src/plugins/](./src/plugins/)**: Fastify decorators and middleware plugins:
  - `mongoosePlugin.ts`: Enforces canonical database naming (`elodb`) and connects Mongoose.
...
- **DB Name Override**: The `mongoosePlugin.ts` dynamically parses the Mongoose connection URI and overrides the path to `/elodb` to ensure consistency.
