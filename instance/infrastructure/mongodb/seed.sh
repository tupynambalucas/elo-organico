...
until mongosh --host instance-db --port 27017 --quiet --eval "db.adminCommand('ping').ok" > /dev/null 2>&1; do
  sleep 2
done
echo "🚀 MongoDB is up."
...
if ! mongosh --host instance-db --port 27017 $AUTH_ARGS --quiet --eval "rs.status().ok" 2>/dev/null | grep -q "1"; then
    echo "🌀 Initiating Replica Set (rs0)..."
    # Usamos o hostname definido no docker network: 'instance-db'
    mongosh --host instance-db --port 27017 $AUTH_ARGS --quiet --eval "rs.initiate({_id: 'rs0', members: [{_id: 0, host: 'instance-db:27017'}]})"
    
    echo "⏱️ Waiting for node to become PRIMARY..."
    until mongosh --host instance-db --port 27017 $AUTH_ARGS --quiet --eval "rs.isMaster().ismaster" 2>/dev/null | grep -q "true"; do
        sleep 2
    done
...
mongosh --host instance-db --port 27017 $AUTH_ARGS --quiet <<EOF
