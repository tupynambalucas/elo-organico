...
This package (`@elo-instance/core`) is the Single Source of Truth (SSOT) for data contracts, validation schemas, and TypeScript interfaces shared across the community instance stack.
...
2. **Core First**: You MUST declare new schemas, parameters, and contract types in this core library before using or referencing them in `@elo-instance/api` or `@elo-instance/web`.
...
- `pnpm build` (or `pnpm --filter @elo-instance/core build`): Compiles TypeScript source files into `dist/`.
