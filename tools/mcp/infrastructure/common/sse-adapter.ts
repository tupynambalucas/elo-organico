...
const ARGS: string[] = process.env.MCP_ARGS ? JSON.parse(process.env.MCP_ARGS) : [];
...
function queryChild(sessionId: string, child: ChildProcessWithoutNullStreams, requestBody: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    let requestMethod = '';
    try {
      const payload = JSON.parse(requestBody) as { id?: string | number; method?: string };
    ...
    console.log(`[Session ${sessionId}] Querying child: method="${requestMethod}", requestId=${requestId}`);
    ...
    const onData = (data: Buffer): void => {
      ...
      console.log(`[Session ${sessionId}] Received stdout chunk (len=${data.length})`);
      ...
      while ((newlineIndex = currentBuffer.indexOf('\n')) !== -1) {
        ...
        if (trimmedLine.length > 0) {
          console.log(`[Session ${sessionId}] Extracted line from stdout (len=${trimmedLine.length}), preview: ${trimmedLine.substring(0, 100)}...`);

          if (requestId === null) {
            console.log(`[Session ${sessionId}] Stateless query or null requestId. Resolving immediately with line.`);
            cleanup();
            resolve(line);
            return;
          }
          ...
          try {
            ...
            if (responsePayload.id === requestId) {
              console.log(`[Session ${sessionId}] Found matching JSON-RPC response for id=${requestId}. Resolving.`);
              cleanup();
              resolve(line);
              return;
            } else {
              console.log(`[Session ${sessionId}] Mismatched JSON-RPC response id=${responsePayload.id} (expected ${requestId}). Continuing to buffer.`);
            }
          } catch (err: unknown) {
            const error = err as Error;
            console.log(`[Session ${sessionId}] Line is not valid JSON (${error.message}). Continuing to buffer.`);
          }
        }
      }
    };
    ...
    try {
      console.log(`[Session ${sessionId}] Writing to child stdin: ${requestBody.trim()}`);
...
fastify.delete<{ Querystring: QueryParams }>('/sse', async (request, reply) => {
  ...
  if (sessionId !== undefined) {
    ...
    if (child !== undefined) {
      console.log(`[HTTP] DELETE request received. Terminating session ${sessionId}`);
      child.kill();
...
fastify.get<{ Querystring: QueryParams }>('/sse', { sse: true }, async (request, reply) => {
  ...
  console.log(`[SSE] Client connected: ${clientId}. Spawning: ${COMMAND} ${ARGS.join(' ')}`);

  const child = spawn(COMMAND, ARGS);
  sessions.set(clientId, child);
  sessionBuffers.set(clientId, '');

  void reply.sse.send({ event: 'endpoint', data: `messages?clientId=${clientId}` });
  ...
  child.on('close', (code) => {
    console.log(`[Subprocess ${clientId}] Terminated with exit code ${code}`);
    reply.raw.end();
  });

  request.raw.on('close', () => {
    console.log(`[SSE] Client disconnected: ${clientId}. Killing subprocess.`);
    child.kill();
...
fastify.post<{ Querystring: QueryParams; Body: string }>('/sse', async (request, reply) => {
  ...
  if (sessionId === undefined) {
    ...
    console.log(`[HTTP] New Streamable session: ${sessionId}. Spawning: ${COMMAND}`);
    ...
    child.on('close', (code) => {
      console.log(`[Subprocess ${sessionId}] Closed with code ${code}`);
      if (sessionId !== undefined) {
        sessions.delete(sessionId);
        sessionBuffers.delete(sessionId);
      }
  ...
  if (isNotification === true) {
    console.log(`[HTTP] Received JSON-RPC Notification: ${body.trim()}. Responding 204 immediately.`);
    try {
      child.stdin.write(body + '\n');
...
const start = async (): Promise<void> => {
  try {
    ...
    console.log(`Fastify v5 SSE Server running on 0.0.0.0:${PORT}`);
  } catch (err: unknown) {
